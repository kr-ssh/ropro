...
function createItem(item, index, noValue) {
	...
	if (id == -1) {
		...
		src = `https://api.ropro.io/getAssetThumbnail.php?id=${parseInt(id)}`
		tooltip = `
...
function leaveItem(container){
	...
	if (itemID == -1) {
		...
		itemImage.src = itemID < -1 ? icons[Math.abs(itemID)] : `https://api.ropro.io/getAssetThumbnail.php?id=${parseInt(itemID)}`
	}
}
		...
		if (currentValue != '') {
			...
			if (this.value == currentValue) {
				...
				for (j = 0; j < items.length; j++) {
					...
					li.innerHTML = `<div style="z-index:1000;margin-top:0px;padding-left:5px;height:30px;position:relative;"><div style="margin-left:0px;display:inline-block;width:255px;text-align:left;overflow:hidden;white-space: nowrap;"><img style="width:30px;margin-left:-4px;margin-right:5px;margin-bottom:3px;" src="https://api.ropro.io/getAssetThumbnail.php?id=${parseInt(items[j].id)}" id="tradeFilterImage"><span id="tradeFilterText">${stripTags(items[j].name)}</span></div></div>`
					list.appendChild(li)
